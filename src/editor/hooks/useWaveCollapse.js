...
const tiles = [
  ...
  },
  {
    type: 'connectRoad',
    rotation: 1,
    sockets: getRotatedSockets(1, [1, 1, 0, 1]),
    up: [],
    right: [],
    down: [],
    left: [],
  },
  {
    type: 'connectRoad',
    rotation: 2,
    sockets: getRotatedSockets(2, [1, 1, 0, 1]),
    up: [],
    right: [],
    down: [],
    left: [], 
  },
  {
    type: 'connectRoad',
    rotation: 3,
    sockets: getRotatedSockets(3, [1, 1, 0, 1]),
    up: [],
    right: [],
    down: [],
    left: [], 
...
const useWaveSockets = () => {
  ...
  const analyzeTile = (tileIndex) => {
    ...
    tiles.forEach((tile, index) => {
      ...
      // UP
      if(tileSockets[2] === testSockets[0]) {
        testTileDomains.up.push(index);
      }

      // RIGHT
      if(tileSockets[3] === testSockets[1]) {
        testTileDomains.right.push(index);
      }

      // DOWN
      if(tileSockets[0] === testSockets[2]) {
        testTileDomains.down.push(index);
      }

      // LEFT
      if(tileSockets[1] === testSockets[3]) {
        testTileDomains.left.push(index);
      }
    });

    console.log(allTileDomains);
