...
const tiles = [
  ...
  },
  {
    type: 'straightH',
    rotation: 0,
    sockets: ['AAA', 'ABA', 'AAA', 'ABA'],
    
  },
  {
    type: 'straightH',
    rotation: 1,
    sockets: getRotatedSockets(1, ['AAA', 'ABA', 'AAA', 'ABA']),
   
  },
];

const useWaveRoads = (gridSize) => {
  /**
   * State Refs
   */
  ...
  const setupGrid = () => {
    for (let i = 0; i < DIM * DIM; i++) {
      waveGrid.current[i] = {
        collapsed: false,
        options: Array.from(tiles, (t, index) => index),
    ...
    for (let i = 0; i < DIM; i++) {
      for (let j = 0; j < DIM; j++) {
        ...
        if(currentGrid[index].collapsed) {
          ...
          let options = Array.from(tiles, (t, index) => index);
